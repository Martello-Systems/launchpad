...
- **Tested.** 93 tests against a real Postgres DB: signup, attribution, leaderboard, position rule, verification flow (incl. scanner-safe `GET`/`POST`-consume + idempotent re-confirm) + token expiry, anti-enumeration (body **and** timing), app-wide security headers, in-memory + Postgres rate limiters (incl. expired-row pruning), streamed CSV export, and embed CSP. CI runs them on every push.
